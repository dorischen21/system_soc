...
class Notice extends CI_Controller {
	...
	public function apiGetDocs() {
		...
		$a = 0;
		foreach ($query->result() as $row) {
			if(mb_strlen($row->name, "utf-8") > '20') {   //判斷字串字數
				$ary_ret[$a]['name'] = mb_substr($row->name,0,20,"utf-8") . '...';
			} else {
				$ary_ret[$a]['name'] = $row->name;
			}
			$ary_ret[$a]['id'] = $row->id ;
			$ary_ret[$a]['company_id'] = $row->company_id ;
			$ary_ret[$a]['projectname'] = $row->projectname ;
			$ary_ret[$a]['create_date'] = $row->create_date ;
			$ary_ret[$a]['number'] = $row->number ;
			$ary_ret[$a]['tw'] = $row->tw ;
	...
	public function sendUser() { 
		...
		$mail_list = explode("\n", $this->company_model->getCompanyMail($this->input->post('hid_companyId')));
