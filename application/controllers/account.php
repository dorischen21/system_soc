...
class Account extends CI_Controller {
	...
	private function getRole($companyId) {
		$user_role = '';
		$this->db->select('role');
		$ary_where = array('company_id' => $companyId, 'account' => $this->input->post('txt_loginAccount'));
	...
	public function doLogin() {
		...
		if($count == 0 || $is_expired) {	// disallow login
			...
			$companyId = $this->user_model->getCompanyIdByUser($this->input->post('txt_loginAccount'));
			$newdata = array(
				'company_id'	=> $companyId,
				'company_name'	=> $this->company_model->getCompanyTw($companyId),
				'account'		=> $this->input->post('txt_loginAccount'),
				'role'			=> $this->getRole($companyId)
			);
